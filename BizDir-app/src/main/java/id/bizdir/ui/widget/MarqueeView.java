...
public class MarqueeView extends LinearLayout {
    ...
    private List<Newsticker> newstickerList;
    private ScrollView mScrollView;
    private int activeIndex = 1;

    private static final int TEXTVIEW_VIRTUAL_WIDTH = 120000;
    ...
    public void setDataNewsTicker(List<Newsticker> newstickerList) {
    ...
    private void prepareAnimation() {
        ...
        mMoveTextOut.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                //Toast.makeText(getContext(),"Habis", Toast.LENGTH_LONG).show();
                ...
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        if (newstickerList.size() > 0) {
                            String title = newstickerList.get(activeIndex).getTitle();
                            if (!TextUtils.isEmpty(title)) {
                                mTextField.setText(Helpers.setTitle(title));
                                mTextField.setTag(activeIndex);
                                if (activeIndex >= newstickerList.size() - 1) {
                                    activeIndex = 0;
                                } else {
                                    activeIndex++;
                                }
